...
export const FloatingCard = ({
    ...
    return (
        <motion.div
            ...
            className={clsx(
                "flex items-center gap-3 sm:gap-4 rounded-full p-4 sm:p-5 lg:px-8 shadow-xl shadow-[#707A99]/10 dark:shadow-none transition-transform duration-300 hover:shadow-2xl hover:shadow-[#707A99]/20",
                colorStyles[color],
                animationClass,
                className
            )}
        >
            {isPortal ? (
                <div className="flex items-center gap-3 pr-2">
                    ...
                    <div className="flex flex-col text-left py-1 flex-1">
                        <span className="font-semibold text-sm lg:text-[15px] text-[#2F253C] dark:text-white leading-tight mb-0.5">{label} - Portal</span>
                        <p className="text-[#636F9D] dark:text-slate-300 text-[10px] lg:text-[11px] leading-tight font-medium max-w-[170px] mix-blend-color-burn dark:mix-blend-normal">
                            Hey! Could you please review a document for me?
                        </p>
                        <span className="text-[#636F9D]/80 dark:text-slate-400 text-[9px] lg:text-[10px] font-medium mt-1">MAT-2233 - 2 h ago</span>
                    </div>
                </div>
            ) : (
                <>
                    ...
                    <span className="font-semibold text-lg lg:text-2xl mt-0.5 whitespace-nowrap">{label}</span>
                </>
            )}
        </motion.div>
    );
